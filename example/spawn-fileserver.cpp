...
public:
    void operator()(asio::yield_context yield)
    {
        ...
        try {
            while (self->socket.is_open()) {
                ...
                http::async_response_transmit_file(self->socket, self->request,
                                                   reply, self->file, yield);
            }
    ...
    static std::shared_ptr<connection> make_connection(asio::io_context &ios,
                                                       ...
                                                       const char *file)
    {
        return std::shared_ptr<connection>{new connection{ios, counter, file}};
    }
...
{
    if (argc != 2) {
        cerr << "Usage:" << endl << argv[0] << " file" << endl;
        return 1;
    }
