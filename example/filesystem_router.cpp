...
public:
    void operator()(asio::yield_context yield)
    {
        ...
        try {
            while (self->socket.is_open()) {
                ...
                {
                    http::message reply;
                    const char body[] = "File does not exist\n";
                    std::copy(body, body + sizeof(body) - 1,
                                std::back_inserter(reply.body()));

                    reply.headers().emplace("Context-Type", "text");

                    self->socket.async_write_response(200, string_ref("OK"),
                                                      reply, yield);
                }
    ...
    void file_route(const char *root,
                    asio::yield_context yield)
    {
        ...
        const size_t BODY_SIZE = 2048;
        char body[BODY_SIZE];
        auto len = snprintf(body, BODY_SIZE,
                            "File exists: %s/%s\n",
                            root, self->path.c_str());

        std::copy(body, body + len,
                    std::back_inserter(reply.body()));
...
#define ROUTER_FUNCTION_ARGUMENTS connection*, \
    ...
    static
    http::filesystem_router<std::function<void(ROUTER_FUNCTION_ARGUMENTS)>,
...
http::filesystem_router<std::function<void(ROUTER_FUNCTION_ARGUMENTS)>,
                       ...
                       > connection::router =
{
    { "/tmp", std::bind(&connection::file_route, _1, "/tmp", _2) },
    { "/home", std::bind(&connection::file_route, _1, "/home", _2) },
};
