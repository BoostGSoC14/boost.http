...
* Just like Ryan Dahl's HTTP parser, this parser does not make any syscalls nor
...
* It doesn't steal control flow from your application footnote:[parsing is done
  ...
  pipelining.
* Matching and decoding token as separate steps footnote:[Proper extraction of
