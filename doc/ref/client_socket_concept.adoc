...
. The `Socket` operations `async_write_trailers` and
  `async_write_end_of_message` are modified so that `write_state::empty` is used
  as a target write state instead `write_state::finished`.
