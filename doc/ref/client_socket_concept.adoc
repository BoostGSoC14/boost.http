...
. Upon receiving a message (i.e. `async_read_response` or `async_read_some`), if
  connection is gracefully closed in the HTTP-equivalent layer
  (e.g. `"connection: close"` header), `ClientSocket` MUST change the state to
  closed (i.e. `is_open()` will return `false`).
+
