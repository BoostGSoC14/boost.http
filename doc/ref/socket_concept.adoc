...
|`a.read_state() == read_state::message_ready \|\| a.read_state() == read_state::body_ready`
a|
Initiate an asynchronous operation to read a part of the message body. Handler
is called when:

* If `a.read_state() == read_state::message_ready`: at least one byte is read
  (called with no error set), or when the end of message is reached (called with
  no error set).
* If `a.read_state() == read_state::body_ready`: the rest of the message is
  fully received (called with no error set).
* When some error occurs.

`m.body()` (if `a.read_state() != read_state::body_ready`) and `m.trailers()`
are left in an unspecified state while the operation is in progress.
|By the time the handler is called, the part of the message read (if any) is
 appended to `m.body()` and the read trailers (if any) are inserted into
 `m.trailers()`, if no error happened.
