...
can be accomplished by the use of structural/notification tokens.
...
The coded control flow will always call `request_reader.next()` when
`on_socket_callback` is called (i.e. before we inspect current token in the
switch-statement). But if 0-sized tokens are possible, then we must exhaust the
current tokens before `on_socket_callback` returns. Otherwise we'll skip/miss
some token from the stream as `request_reader.next()` always consume the current
token.
